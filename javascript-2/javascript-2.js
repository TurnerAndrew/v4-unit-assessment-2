...
//   let cals = 0
//   foods.forEach((carbs, protein, fat) => {
...
let saleProducts = products.map(element => ({name: element.name, color: element.color, price: element.price * .75}))
...
const blueProducts = saleProducts.filter((item) => item.color.includes('blue'))
...
const orderTotal = blueProducts.reduce((acc, element) => acc += element.price)
...
userInfo.settings.shouldAlert = userInfo.settings.alerts
